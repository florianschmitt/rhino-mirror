...
/**
 * This class is the Rhino / Spidermonkey regexp engine.
 *
 * Revision History:
 ...
 * Rhino regexp engine separated from NativeRegExp object to allow other
 *  implementations
 *
 * @author Brendan Eich
 * @author Norris Boyd
 * @author Joel Hockey
 */
public class RERhino implements RegExpEngine, Serializable {
    private static final long serialVersionUID = 0xBC10458D5F6772A1L;
    ...
    private RECompiled regexp;
    private REGlobalData gData;
    ...
    public RERhino(Context cx, String source, boolean global,
        boolean ignoreCase, boolean multiline, boolean flat) {
        this.source = source;
        this.global = global;
        this.ignoreCase = ignoreCase;
        this.multiline = multiline;

        regexp = new RECompiled();
        regexp.source = source.toCharArray();

        regexp.flags = (global ? JSREG_GLOB : 0) | (ignoreCase ? JSREG_FOLD : 0) |
            (multiline ? JSREG_MULTILINE : 0);

        CompilerState state = new CompilerState(cx,
                regexp.source, source.length(), regexp.flags);

        if (flat && (source.length() > 0)) {
            if (debug) {
                System.out.println("flat = \"" + source + "\"");
            }

            state.result = new RENode(REOP_FLAT);
            state.result.chr = state.cpbegin[0];
            state.result.length = source.length();
            state.result.flatIndex = 0;
            state.progLength += 5;
        } else {
//            if (!parseDisjunction(state))
//                return null;
// no need to check return since error throws Exception
    ...
    public boolean find(int start, boolean forceMultiline) {
        gData = new REGlobalData();
        regionStart = start;

        int end = chars.length;

        if (regexp.parenCount != 0) {
            gData.parens = new long[regexp.parenCount];
        } else {
            gData.parens = null;
        }
        ...
        gData.multiline = forceMultiline;
        gData.regexp = regexp;
        gData.lastParen = 0;
    ...
    public int start() {
        return regionStart + gData.skipped;
    }
    ...
    public String group(int group) {
        ...
        group--; // parens are 0-based

        int start = gData.parens_index(group);
        if (start == -1) {
            return null;
        }
        int end = start + gData.parens_length(group);
    ...
    private static int getDecimalValue(char c, CompilerState state,
        ...
        if (overflow) {
            NativeRegExp.reportError(overflowMessageId,
        ...
        case '\\':
            ...
            if (state.cp < state.cpend) {
                ...
                case 'x': {
                    ...
                    for (i = 0; (i < nDigits) && (state.cp < state.cpend);
                            i++) {
                        c = src[state.cp++];
                        n = Kit.xDigitToInt(c, n);
            ...
            if (NativeRegExp.isDigit(c)) {
                ...
                if (c == ',') {
                    ...
                    if (NativeRegExp.isDigit(c)) {
                        ...
                        if (min > max) {
                            NativeRegExp.reportError("msg.max.lt.min",
    ...
    public static boolean matchRegExp(REGlobalData gData, RECompiled re,
